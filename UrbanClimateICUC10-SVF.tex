...
%Urban heat is a significant health risk, especially during extreme heat days. Identifying vulnerable areas of cities is a priority for devising effective heat mitigation strategies. However, making these observations is difficult, expensive, and time consuming. The platform developed through this project allows systematic assessments to be made for any urban area using a global and consistent dataset, Google Street View (GSV) imagery.


Computer vision techniques allow automated detection of sky pixels in outdoor imagery. There are multiple applications for this information across a large number of research areas. In urban climate, it is an important first step in gathering information about urban morphologies and sky view factors (SVF). However, despite years of research and a multitude of techniques, obtaining accurate results is still challenging. This problem becomes even more challenging using imagery captured under a variety of lighting and weather conditions. 

To address this problem, we present a new sky pixel detection system demonstrated to produced accurate results using a wide range of outdoor imagery type. This imagery is processed using an ensemble of mean-shift segmentation, K-means clustering, and Sobel filters to mark sky pixels in the image and extract sky view factors (SVF). The images are pre-classified prior to clustering by a convolutional neural network (CNN), trained with 40,000 images from the Skyfinder dataset \citep{Mihail2016} and from Google Street View (GSV) \citep{GoogleMaps2017b}. This pre-classification step allows the sky marking to follow an adaptive process and to use different techniques and parameters to choose the technique that can best process a particular image. An evaluation of fourteen different combinations of techniques and parameters shows that no single technique can perform with high accuracy. However, by using our adaptive process, large increases in accuracy are seen, far exceeding many of the pre-existing techniques.
...
\begin{keyword}
sky view factor \sep Google Street View \sep machine learning \sep WUDAPT \sep sky pixel detection

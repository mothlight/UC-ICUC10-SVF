...
All the technique and parameter variations were used to process the two data sets of the 38,115 Skyfinder and 406 GSV images. A summary of $d$ index of agreement \citep{Willmott1981}, R$^{2}$, and root-mean-square error (RMSE) statistics for evaluations against the two datasets is presented in Table \ref{tab:evalall}. Plots of a number of the better performing techniques are presented in Figure \ref{fig:errorallcombined}. Note, the strong horizontal lines in these figures are due to the nature of the Skyfinder data set that contains large groups of the same scenes (with the same percentage of sky) under different lighting and weather conditions, often resulting in a wide range of calculated results.
...
Code and data are available from the corresponding author on request. They are also provided in the linked Data in Brief article \citep{Nice2019Data} and at https://doi.org/10.5281/zenodo.2562396.
